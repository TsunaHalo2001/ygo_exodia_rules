...
class YGOCard {
  ...
  static Future<Map<int, YGOCard>> genNormalMonsters(List<dynamic> data) async {
    return Isolate.run(() {
      var cards = <int, YGOCard>{};

      for (var item in data) {
        if (!item['type'].contains('Normal Monster')) {
          continue;
        }
  ...
  static Future<Map<int, YGOCard>> genFusionMonsters(List<dynamic> data) async {
    return Isolate.run(() {
      var cards = <int, YGOCard>{};

      for (var item in data) {
        if (!item['type'].contains('Fusion Monster')) {
          continue;
        }
